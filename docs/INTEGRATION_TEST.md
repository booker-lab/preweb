...
- [ ] `apps/api/.env` 파일에 실제 값 등록 확인 (Firebase, JWT, 포트원)
- [ ] `apps/consumer/.env.local` 파일에 실제 값 등록 확인
...
| 2-5 | 상품 목록 | `GET /products/group-products` (Bearer 토큰) | 배열 200 |
...
- [ ] `pnpm --filter api build` 오류 없음
- [ ] `pnpm --filter consumer build --webpack` 오류 없음
- [ ] `CORS_ORIGIN`에 Vercel 프로덕션 도메인 추가 예정
- [ ] Railway 환경변수에 `FIREBASE_SERVICE_ACCOUNT_JSON` Secret 등록 예정
- [ ] Vercel 환경변수에 `NEXT_PUBLIC_*` + `NEXTAUTH_SECRET` 등록 예정
