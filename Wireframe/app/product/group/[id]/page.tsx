...
export default function GroupBuyDetailPage() {
  ...
  const progressPercent = (product.groupBuy.current / product.groupBuy.total) * 100

  const handleParticipate = () => {
    setIsSheetOpen(false)
    // TODO: Implement participate logic
  ...
  return (
    <div className="min-h-screen bg-background max-w-[390px] mx-auto relative pb-24">
      ...
      <header className="fixed top-0 left-1/2 -translate-x-1/2 w-full max-w-[390px] bg-card/95 backdrop-blur-sm z-50 border-b border-border">
        <div className="flex items-center justify-between h-14 px-4">
          ...
          <h1 className="text-sm font-medium text-foreground truncate max-w-[180px]">
            {scrolledPastImages ? product.name : "공동구매 상세"}
          </h1>
      ...
      <main className="pt-14">
        ...
        {/* Delivery Details for Group Buy */}
        <div className="mx-4 mb-4 bg-muted rounded-xl p-4">
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {product.groupBuy.deliveryDate} (판매자 지정)
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {product.groupBuy.deliveryMethod} (판매자 지정)
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {formatPrice(product.groupBuy.deliveryFee)}원 (일반 대비 할인)
                </p>
              </div>
            </div>
        ...
        {/* Group Buy Progress */}
        <div className="mx-4 mb-4 bg-accent/30 rounded-xl p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-destructive font-bold">
              마감까지 D-{product.groupBuy.daysLeft}
            </span>
          </div>
          ...
          <p className="text-sm text-muted-foreground text-center">
            <span className="text-primary font-bold">{product.groupBuy.current}</span>/{product.groupBuy.total}명 참여중
          </p>
        </div>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[390px] bg-card border-t border-border z-50 px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="flex-1 h-12 bg-primary text-primary-foreground rounded-xl font-bold text-sm flex flex-col items-center justify-center"
          >
            <span>공동구매 참여하기</span>
            <span className="text-xs font-normal opacity-90">
              {formatPrice(product.salePrice)}원 · 마감 D-{product.groupBuy.daysLeft}
            </span>
          </button>
        </div>
      ...
      <GroupBuyOptionSheet
        ...
        deliveryDate={product.groupBuy.deliveryDate}
        deliveryMethod={product.groupBuy.deliveryMethod}
        deliveryFee={product.groupBuy.deliveryFee}
        currentParticipants={product.groupBuy.current}
        totalParticipants={product.groupBuy.total}
        daysLeft={product.groupBuy.daysLeft}
